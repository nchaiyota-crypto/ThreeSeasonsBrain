  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        const mergedServiceFee =
          typeof supa?.serviceFee === "number"
            ? supa.serviceFee
            : typeof (last as any)?.serviceFee === "number"
              ? (last as any).serviceFee
              : 0;
        ...
        <div style={{ borderTop: "1px solid #e9e9e9", paddingTop: 12 }}>
          ...
          {(order.serviceFee ?? 0) > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              ...
              <div style={{ fontWeight: 900 }}>${(order.serviceFee ?? 0).toFixed(2)}</div>
            </div>
          )}
