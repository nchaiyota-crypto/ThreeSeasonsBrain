...
const BUSINESS_HOURS: Record<DayKey, Hours> = {
  ...
  6: { closed: true, open: "00:00", close: "00:00" }, // Sat
};
...
function getEffectiveWaitMinutes(ws: GetWaitStatusResponse | null, fallback: number) {
  if (!ws) return fallback;
  if (ws.paused) return fallback; // UI handles paused separately
...
export default function MenuPage() {
  ...
  const orderingPaused = ws?.paused === true;
    ...
    let alive = true;
      ...
      async function load() {
        try {
          ...
          setWs(data);
          const effective = getEffectiveWaitMinutes(data, DEFAULT_ESTIMATE_MIN);
